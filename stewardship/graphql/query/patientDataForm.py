...
class PatientDataFormQuery(graphene.ObjectType):
    ...
    patientsForm = graphene.List(PatientDataFormObj, patientId=graphene.ID(required=True),date = graphene.String(required=True))
    allClinicalSigns = graphene.List(PatientDataFormObj, startDate=graphene.String(required=True),endDate = graphene.String(required=True))
